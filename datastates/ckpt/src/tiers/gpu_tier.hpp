...
class gpu_tier_t : public base_tier_t {
    ...
    cudaEvent_t flush_complete_event;
    cudaEvent_t fetch_complete_event;
...
public:
    ...
    ~gpu_tier_t() {
        ...
        // Cleanup events
        if (flush_complete_event) cudaEventDestroy(flush_complete_event);
        if (fetch_complete_event) cudaEventDestroy(fetch_complete_event);
    };
